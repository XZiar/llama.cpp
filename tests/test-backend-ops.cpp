...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    test_cases.emplace_back(new test_flash_attn_ext(128, 128, 16, {1, 1}, 5776, 5776, false, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16));
    ...
    for (const auto kvt : { GGML_TYPE_F16, GGML_TYPE_Q8_0, }) {
        for (int kv : { 4096, 8192, 16384, }) {
            for (int hs : { 64, 128, }) {
                for (int nh : { 8, 16, }) {
                    for (int nr : { 1, 4, }) {
                        test_cases.emplace_back(new test_flash_attn_ext(hs, hs, nh, {nr, 1}, kv, 1, true, false, 0, 0, GGML_PREC_F32, kvt));
                    }
                }
            }
        }
