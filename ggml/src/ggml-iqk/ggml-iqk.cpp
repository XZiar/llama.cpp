...
static void ggml_iqk_quantize_row_q8_K_ref(const float * src, void * dst, int64_t n) {
...
struct ggml_backend_iqk_context {
    ...
    ggml_from_float_t cpu_quantize_row_q8_K = ggml_iqk_quantize_row_q8_K_ref;
};
...
static void ggml_iqk_quantize_row(enum ggml_type type, const float * src, void * dst, int64_t n,
        ggml_from_float_t quantize_row_q8_K) {
    if (type == GGML_TYPE_Q8_2_X4) {
        quantize_row_q8_2_x4(src, dst, n);
    } else if (type == GGML_TYPE_Q8_K128) {
        quantize_row_q8_K128_ref(src, (block_q8_K128 *) dst, n);
    } else {
        GGML_ASSERT(quantize_row_q8_K != nullptr);
    ...
    for (int64_t i13 = 0; i13 < src1->ne[3]; ++i13) {
        for (int64_t i12 = 0; i12 < src1->ne[2]; ++i12) {
            for (int64_t i11 = ith; i11 < src1->ne[1]; i11 += nth) {
                ...
                ggml_iqk_quantize_row(type, src, dst, src1->ne[0], ctx->cpu_quantize_row_q8_K);
            }
        }
    }
...
static void ggml_backend_iqk_assign_cpu_reg(ggml_backend_t backend_iqk, ggml_backend_reg_t cpu_reg) {
    ...
    ctx->cpu_quantize_row_q8_K = ggml_iqk_quantize_row_q8_K_ref;
    if (cpu_reg) {
        ...
        if (ctx->cpu_get_type_traits) {
            ...
            if (traits && traits->from_float) {
                ...
                GGML_LOG_INFO("IQK: cpu_quantize_row_q8_K registered\n");
            }
        }
    }
}
